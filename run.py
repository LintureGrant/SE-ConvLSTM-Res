...
from model import SE_ConvLstm_Res
import torch
import time
...
if __name__ == "__main__":
    ...
    # -----model------------------------
    model_res = SE_ConvLstm_Res.ConvLSTM(input_dim=input_dim, hidden_dim=hidden_dim, kernel_size=kernel_size, num_layers=num_layers, data_row_dim=[len(data[0]), len(data[0][0])],
                                               batch_first=True, res_rate=1).to(device)
    optimizer_res = torch.optim.Adam(model_res.parameters(), lr=lr)
    scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer_res, gamma=gamma)
    ...
    for i in range(40):
        ...
        for j, item in enumerate(train_data):
            ...
            out = model_res(train_x.unsqueeze(0))
            # print(out.shape,train_y.shape)
            loss_res = loss_function(out, train_y)
            loss_batch += loss_res.item()
            loss_train_batch += loss_res.item()
            torch.autograd.backward(loss_res)
            optimizer_res.step()
            optimizer_res.zero_grad()
            if (j + 1) % 100 == 0:
                print('Epoch: {},batch:{}, Loss:{:.5f}'.format(i + 1, j + 1, loss_batch / 50))  # loss_test:{:.5f}
                loss_batch = 0
        losses_res.append(loss_train_batch / len(train_data))
        scheduler.step()
        with torch.no_grad():
            model_res.eval()
            for n, item in enumerate(test_data):
                test_x, test_y = item
                out_test = model_res(test_x.unsqueeze(0))
                loss_test = loss_function(out_test, test_y)
                loss_test_batch += loss_test.item()
                # record validation loss
                # print ("validloss: {:.6f},  epoch : {:02d}".format(loss_aver,epoch),end = '\r', flush=True)
            losses_test_res.append(loss_test_batch / len(test_data))
